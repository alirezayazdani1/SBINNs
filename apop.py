...
#        self.S_scale = tf.map_fn(lambda x: x, self.S_data_std[3:4])
    ...
    def SysODE(self, S, t):
#        self.S_scale = 0.99*self.S_scale + 0.01*tf.math.reduce_std(S, 0)
...
if __name__ == "__main__": 
    ...
    model.train(num_epochs=15000, batch_size=N_eqns//2, learning_rate=1e-3)
    model.train(num_epochs=15000, batch_size=N_eqns//2, learning_rate=1e-4)
    model.train(num_epochs=10000, batch_size=N_eqns//2, learning_rate=1e-5)
